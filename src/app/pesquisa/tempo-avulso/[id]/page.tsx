...
// As 96 afirmações da Rosa Krausz
...
export default function PesquisaTempoAvulsa() {
  ...
  useEffect(() => {
    const buscarAvaliado = async () => {
      ...
      setAvaliado(data);
      if (data.js_respostas && data.js_respostas.length > 0) {
        setRespostas(data.js_respostas);
        // Assumindo que se tem algo no array, marcamos como respondido pra facilitar
      }
      setLoading(false);
    };
    buscarAvaliado();
  ...
  const marcarResposta = async (index: number, valor: boolean) => {
    ...
    // Salva o progresso parcial a cada clique (Em andamento)
    if (avaliado.tp_status === "PENDENTE") {
      await supabase
        .from("AVALIACOES_TEMPO_AVULSO")
        .update({ tp_status: "EM_ANDAMENTO" })
        .eq("cd_avaliacao", params.id);
      setAvaliado({ ...avaliado, tp_status: "EM_ANDAMENTO" });
    }
  };
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center font-bold text-[#064384] animate-pulse">
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        ...
        <p className="text-slate-500 text-center">
          Verifique se o link possui o formato correto.
        </p>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center font-sans">
        ...
        <p className="text-slate-500 max-w-md">
          Obrigado, <b>{avaliado.nm_avaliado}</b>. Suas respostas foram salvas e
          o consultor já foi notificado.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-24">
      ...
      <header className="bg-white border-b border-slate-200 p-6 text-center sticky top-1.5 z-40 shadow-sm">
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-lg p-4 mt-5 max-w-2xl mx-auto">
          <p className="text-xs text-slate-600 font-medium leading-relaxed">
            Se a afirmação descrever o seu comportamento mais frequente,
            assinale <b>"SIM"</b>, se não, assinale <b>"NÃO"</b>.
          </p>
        </div>
      </header>
            ...
            <div className="flex gap-2 shrink-0">
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-black text-sm transition-all border-2 w-24 flex items-center justify-center gap-1
                  ${respondidas[index] && respostas[index] === true ? "bg-[#064384] text-white border-[#064384] shadow-md scale-105" : "bg-slate-50 text-slate-400 border-slate-200 hover:border-[#064384] hover:text-[#064384]"}`}
              >
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-black text-sm transition-all border-2 w-24 flex items-center justify-center gap-1
                  ${respondidas[index] && respostas[index] === false ? "bg-red-500 text-white border-red-500 shadow-md scale-105" : "bg-slate-50 text-slate-400 border-slate-200 hover:border-red-500 hover:text-red-500"}`}
              >
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-sm border-t border-slate-200 p-4 px-6 flex justify-center z-50">
        <button
          onClick={enviarPesquisa}
          disabled={isSaving || progresso < 100}
          className="w-full max-w-md bg-[#FF8323] hover:bg-orange-600 text-white px-8 py-4 rounded-xl font-black shadow-lg disabled:opacity-60 active:scale-95 transition-all flex items-center justify-center gap-2"
        >
          {isSaving ? (
            ...
            <>
              {" "}
              <span className="material-symbols-outlined">send</span> Finalizar
              Inventário{" "}
            </>
          )}
        </button>
