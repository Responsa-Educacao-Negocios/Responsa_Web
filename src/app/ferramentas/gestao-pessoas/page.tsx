...
const AREAS = [
  {
    ...
    perguntas: [
      "Você possui descrições de cargos detalhadas e atualizadas para as principais posições na empresa?",
      "Você possui um processo estruturado para recrutar e selecionar novos colaboradores?",
      "Você utiliza ferramentas de avaliação para avaliar as competências dos candidatos?",
      "Você verifica referências e antecedentes antes de contratar novos colaboradores?",
    ],
  },
  {
    id: "treinamento",
    ...
    perguntas: [
      "Você oferece treinamento para o desenvolvimento dos colaboradores?",
      "Você realiza avaliações para identificar as necessidades de treinamento da equipe?",
      "Você incentiva os colaboradores a participarem de cursos externos?",
      "Você proporciona oportunidades de rotação de funções ou atividades para ampliar as habilidades dos colaboradores?",
    ],
  },
  {
    id: "engajamento",
    nome: "Engajamento e Clima Organizacional",
    perguntas: [
      "Você realiza pesquisas para avaliar o nível de satisfação dos colaboradores?",
      "Você promove atividades e eventos para fortalecer o engajamento da equipe?",
      "Você incentiva a comunicação aberta entre os colaboradores e a liderança?",
      "Você reconhece e recompensa o bom desempenho dos colaboradores?",
    ],
  },
  {
    id: "desempenho",
    ...
    perguntas: [
      "Você realiza avaliações de desempenho regulares com todos os colaboradores?",
      "Você define metas claras e mensuráveis para a equipe?",
      "Você oferece feedback construtivo e frequente para os colaboradores?",
      "Você utiliza os resultados das avaliações de desempenho para orientar promoções e aumentos salariais?",
    ],
  },
  {
    id: "beneficios",
    nome: "Benefícios e Remuneração",
    perguntas: [
      "Você oferece salários competitivos em relação ao mercado local para atrair e reter talentos?",
      "Você possui uma política clara de aumentos salariais baseada no desempenho dos colaboradores?",
      "Você proporciona benefícios básicos, como vale-transporte ou auxílio alimentação, para apoiar seus colaboradores?",
      "Você oferece alguma forma de benefício adicional, como horários flexíveis ou apoio para desenvolvimento profissional?",
    ],
  },
];
...
const NIVEIS = [
  { min: 0, max: 30, nome: "Início da Jornada", descricao: "A gestão de pessoas está em fase inicial. É urgente estruturar processos básicos de RH, pois a ausência de boas práticas representa risco para a retenção e engajamento dos colaboradores.", cor: "#ef4444" },
  { min: 31, max: 45, nome: "Em Desenvolvimento", descricao: "A empresa já possui algumas boas práticas de gestão de pessoas, mas há espaço significativo para estruturar processos mais robustos e consistentes.", cor: "#f59e0b" },
  { min: 46, max: 60, nome: "Alto Desempenho", descricao: "A empresa possui uma gestão de pessoas madura e estruturada. O foco agora deve ser na inovação contínua e no fortalecimento da cultura organizacional.", cor: "#10b981" },
];
...
export default function GestaoPessoasPage() {
  ...
  const gerarPDF = () => {
    ...
    <title>Diagnóstico de Gestão de Pessoas — ${ficha.razaoSocial}</title>
    <style>
      ...
      .cover{width:210mm;min-height:297mm;background:linear-gradient(135deg,#0f4c2e 0%,#166534 60%,#15803d 100%);display:flex;flex-direction:column;justify-content:space-between;padding:60px 50px;page-break-after:always}
      ...
      .cover-title{color:white;font-size:40px;font-weight:900;line-height:1.1;margin-bottom:12px}
    ...
    <div class="cover">
      ...
      <div class="cover-center">
        ...
        <div class="cover-title">Diagnóstico de<br/>Gestão de Pessoas</div>
        <div class="cover-sub">Avaliação das Práticas de RH Organizacional</div>
      ...
      <div class="cover-footer">Diagnóstico de Gestão de Pessoas · Gerado pela Plataforma RESPONSA · Confidencial</div>
    </div>

    <div class="page">
      <div class="ph"><span class="ph-logo">RESPONSA · Gestão de Pessoas</span><span class="ph-info">${ficha.razaoSocial} · ${dataFmt}</span></div>
    ...
    <div class="page">
      ...
      <h2>Pontuação e Nível de Maturidade</h2>
    ...
    <div class="page">
      ...
      <p>As áreas abaixo apresentam os menores índices de maturidade em gestão de pessoas e devem receber atenção prioritária:</p>
      <table>
        <tr><th>#</th><th>Área</th><th>Pontuação</th><th>% de Maturidade</th></tr>
      ...
      <div style="margin-top:48px;text-align:center;padding:20px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;">
        <p style="font-size:12px;color:#94a3b8;font-weight:600;">
          Relatório gerado pela Plataforma <strong style="color:#064384;">RESPONSA</strong> · Diagnóstico de Gestão de Pessoas<br/>
    ...
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col h-full">
        <header className="bg-white px-8 py-5 border-b border-slate-200 shadow-sm sticky top-0 z-10 flex items-center gap-4">
          <div className="size-12 bg-green-50 text-green-600 rounded-xl flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-[26px]" style={{ fontVariationSettings: "'FILL' 1" }}>groups_3</span>
          </div>
          <div>
            <h2 className="text-xl font-black text-[#064384]">Atendimento — Gestão de Pessoas</h2>
            <p className="text-sm font-bold text-slate-500">Diagnóstico de Maturidade em Gestão de Pessoas</p>
          </div>
        </header>
          ...
          <div className="flex items-center gap-1 overflow-x-auto">
            {STEPS.map((s, i) => (
              <div key={s.id} className="flex items-center gap-1 shrink-0">
                <button onClick={() => i < stepIdx && setStep(s.id)}
            ...
            return (
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setStep("melhoria")} className="flex items-center gap-2 h-12 px-8 bg-[#064384] text-white font-black rounded-xl hover:bg-blue-900 transition-all shadow-md">Áreas de Melhoria <span className="material-symbols-outlined text-[18px]">trending_down</span></button>
                </div>
              </div>
            );
          })()}
          ...
          {step === "plano" && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
                ...
                <p className="text-sm text-slate-500 font-medium mb-6">Registre até 5 recomendações para o desenvolvimento das áreas identificadas.</p>
                <div className="space-y-4">
                  {plano.map((rec, i) => (
                    <div key={i} className="flex gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <textarea rows={2} placeholder={`Descreva a recomendação ${i + 1}...`} value={rec}
            ...
            return (
              <div className="space-y-6">
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center">
                  ...
                  <button onClick={() => { setStep("ficha"); setRespostas({}); setPlano(["", "", "", "", ""]); }} className="mt-4 flex items-center justify-center gap-2 h-12 px-8 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-all mx-auto">
