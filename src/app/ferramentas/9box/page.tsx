...
const STORAGE_KEY = "9box_avulso";

// ── Component ─────────────────────────────────────────────────────────
export default function NoveBoxFerramentaPage() {
  ...
  const handleDelete = (id: string) => persistir({ ...dados, [tipo]: colabsDoTipo.filter(c => c.id !== id) });
  const cancelar = () => { setForm(FORM_VAZIO); setEditId(null); setMostrarForm(false); };

  const getColabsByBox = (comp: Nivel, eixoVal: Nivel) =>
    colabsDoTipo.filter(c => c.comportamento === comp && c.eixo === eixoVal);
...
<h2 style="font-size:12px;font-weight:900;color:#064384;text-transform:uppercase;border-bottom:2px solid #064384;padding-bottom:6px;margin-bottom:10px;">Listagem (${colabsDoTipo.length})</h2>
<table><thead><tr>
  <th style="width:36px;">#</th><th>Nome</th><th>Cargo</th><th>Departamento</th><th>Comportamento</th><th>${cfg.eixoLabel}</th><th>Classificação</th>
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col h-full">
        {/* Header */}
        <header className="bg-white/95 backdrop-blur-sm px-8 py-6 border-b border-slate-200 shadow-sm sticky top-0 z-10 flex items-center gap-4 shrink-0">
          <div className="size-12 bg-violet-50 text-violet-600 rounded-xl flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>grid_view</span>
          </div>
          <div className="flex-1">
            <h2 className="text-2xl font-black text-[#064384] tracking-tight">Matriz 9 Box</h2>
            <p className="text-sm font-bold text-slate-500">Avaliação Comportamental (I Sólides)</p>
          </div>
          <button
            onClick={exportarPDF}
        ...
        {/* Tabs */}
        <div className="bg-white border-b border-slate-200 px-8 flex gap-1 pt-3">
          {(Object.keys(TIPOS_CONFIG) as TipoFormulario[]).map(t => (
            ...
            >
              ...
              <span className="bg-slate-200 text-slate-600 text-[10px] font-black px-1.5 py-0.5 rounded-full">{dados[t].length}</span>
            </button>
          ))}
        </div>

        {/* Body */}
        <div className="flex-1 overflow-y-auto p-8 space-y-6 max-w-[1400px] mx-auto w-full">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium mt-0.5">Y: Comportamento &nbsp;|&nbsp; X: {cfg.eixoLabel}</p>
              </div>
              {!mostrarForm && (
                <button onClick={() => setMostrarForm(true)} className="flex items-center gap-2 bg-[#064384] hover:bg-blue-900 text-white text-sm font-bold px-4 py-2 rounded-xl transition-all">
            ...
            {mostrarForm && (
              <form onSubmit={handleSubmit} className="p-6 border-b border-slate-100 bg-slate-50/50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div>
                    <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Nome *</label>
                    <input required value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))}
                      className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-[#064384] transition-all"
                  ...
                  <div>
                    ...
                    <select value={form.comportamento} onChange={e => setForm(f => ({ ...f, comportamento: e.target.value as Nivel }))}
                      className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-[#064384] appearance-none cursor-pointer">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="flex gap-2">
                      {NIVEIS.map(n => (
                        <button key={n} type="button" onClick={() => setForm(f => ({ ...f, eixo: n }))}
                ...
                <div className="flex gap-3 justify-end">
                  <button type="button" onClick={cancelar} className="px-5 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-100 rounded-xl transition-colors">Cancelar</button>
            ...
            {colabsDoTipo.length > 0 ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50">
                      {["#", "Nome", "Cargo", "Departamento", "Comportamento", cfg.eixoLabel, "Quadrante", ""].map(h => (
                        <th key={h} className="text-left px-5 py-3 text-[11px] font-black text-slate-500 uppercase tracking-widest">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <span className={`text-[10px] font-black px-2 py-1 rounded-full ${c.comportamento === "Acima" ? "bg-emerald-50 text-emerald-700" : c.comportamento === "Dentro" ? "bg-blue-50 text-blue-700" : "bg-red-50 text-red-600"}`}>{c.comportamento}</span>
                          </td>
                          <td className="px-5 py-3">
                            <span className={`text-[10px] font-black px-2 py-1 rounded-full ${c.eixo === "Acima" ? "bg-emerald-50 text-emerald-700" : c.eixo === "Dentro" ? "bg-blue-50 text-blue-700" : "bg-red-50 text-red-600"}`}>{c.eixo}</span>
                          </td>
                          <td className="px-5 py-3">
                            <span className="text-[10px] font-black px-2 py-1 rounded-full border" style={{ color: box.cor, background: box.bg, borderColor: box.cor + "40" }}>{box.nome}</span>
                          </td>
                          <td className="px-5 py-3">
                            <div className="flex gap-1">
                              <button onClick={() => handleEdit(c)} className="p-1.5 text-slate-400 hover:text-[#064384]"><span className="material-symbols-outlined text-[16px]">edit</span></button>
                              <button onClick={() => handleDelete(c.id)} className="p-1.5 text-slate-400 hover:text-red-500"><span className="material-symbols-outlined text-[16px]">delete</span></button>
                            </div>
                          </td>
                        </tr>
                      );
              ...
              <div className="text-center py-10 text-slate-400">
                ...
                <p className="text-sm font-medium">Adicione colaboradores para preencher a matriz.</p>
                {!mostrarForm && <button onClick={() => setMostrarForm(true)} className="mt-3 text-[#064384] font-bold text-sm hover:underline">Adicionar primeiro</button>}
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-black text-slate-800 mb-1">Matriz 9 Box Visual</h3>
            <p className="text-xs text-slate-400 font-medium mb-6">Y: Comportamento &nbsp;|&nbsp; X: {cfg.eixoLabel}</p>
            <div className="overflow-x-auto">
              <div className="min-w-[600px]">
                <div className="grid gap-2 mb-2" style={{ gridTemplateColumns: "120px repeat(3, 1fr)" }}>
                  <div />
                  {NIVEIS.map(n => (
                    <div key={n} className="text-center text-[10px] font-black text-slate-500 uppercase tracking-widest py-1.5 bg-slate-100 rounded-lg">{n} das Expectativas</div>
                  ))}
                </div>
                <div className="grid gap-2 mb-4" style={{ gridTemplateColumns: "120px repeat(3, 1fr)" }}>
                  <div />
                  <div className="col-span-3 text-center text-[10px] font-black text-[#064384] uppercase tracking-widest opacity-60">← {cfg.eixoLabel} →</div>
                </div>
                {(["Acima", "Dentro", "Abaixo"] as Nivel[]).map(comp => (
                  <div key={comp} className="grid gap-2 mb-2" style={{ gridTemplateColumns: "120px repeat(3, 1fr)" }}>
                    <div className="flex items-center justify-center text-center text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-100 rounded-lg px-2 py-3">
                      {comp}<br /><span className="text-[8px] font-medium normal-case tracking-normal opacity-60">Expectativas</span>
                    </div>
                    {NIVEIS.map(eixoVal => {
                      const box = MATRIZ[comp][eixoVal];
                      const pessoas = getColabsByBox(comp, eixoVal);
                      return (
                        <div key={eixoVal} className="rounded-xl border-2 p-3 min-h-[100px] relative" style={{ background: box.bg, borderColor: box.cor + "50" }}>
                          <div className="flex items-start justify-between mb-2">
                            <span className="text-[9px] font-black uppercase leading-tight" style={{ color: box.cor }}>{box.nome}</span>
                            <span className="text-xl font-black opacity-10" style={{ color: box.cor }}>{box.num}</span>
                          </div>
                          <div className="space-y-1">
                            {pessoas.map(p => (
                              <div key={p.id} className="text-[11px] font-bold bg-white/80 rounded px-2 py-1 text-slate-700 shadow-sm truncate">{p.nome}</div>
                            ))}
                            {pessoas.length === 0 && <div className="text-[10px] text-slate-300 italic">—</div>}
                          </div>
                        </div>
                      );
                    })}
